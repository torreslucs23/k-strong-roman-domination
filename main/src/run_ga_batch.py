...
GRAPH_DIRS = [
    "geoinstances_filtered",
    "Harwell-Boeing_filtered",
    "random_filtered",
    "wireless-networks_filtered",
    "DIMACS_filtered",
]

OUTPUT_CSV = "ga_results.csv"
CHECKPOINT_FILE = ".checkpoint_ga.txt"
BACKUP_INTERVAL = 5
N_RUNS = 20
...
def run_ga_multiple_times(G, n_runs=20):
    ...
    for run in range(n_runs):
        ...
        ga = GeneticAlgorithm(
            ...
            gene_mutation_rate=BEST_PARAMS['gene_mutation_rate'],
            n_mutants=BEST_PARAMS['n_mutants'],
            crossover_rate=BEST_PARAMS['crossover_rate'],
            fix_instance=fix_instance,
            elitism_rate=BEST_PARAMS['elitism_rate']
        )
        
        start_time = time.time()
        final_population = ga.run(generations=BEST_PARAMS['generations'])
        end_time = time.time()
        ...
        if len(final_population) > 0:
            best_individual = min(final_population, key=lambda x: sum(x))
            fitness = sum(best_individual)
            results.append(fitness)
        else:
            results.append(float('inf'))
        
        print(f"    Run {run+1}/{n_runs}: fitness={results[-1]:.0f}, time={elapsed_ms:.2f}ms")
...
def run():
    print("=" * 80)
    print("GA BATCH TESTING - MULTIPLE RUNS PER GRAPH")
    print("=" * 80)
    ...
    for graph_type in GRAPH_DIRS:
        ...
        for filename in files:
            ...
            try:
                ...
                stats = run_ga_multiple_times(G, n_runs=N_RUNS)
